...
<body class="antialiased min-h-screen flex flex-col selection:bg-[#DCD3C3] selection:text-[#1C1B18]">
            ...
            <div class="bg-[#FFFFFF] p-5 sm:p-6 rounded-2xl sm:rounded-3xl border border-[#DCD4C5] shadow-xs">
                <div class="flex items-center justify-between mb-4 flex-wrap gap-2">
                    <div>
                        ...
                        <p class="text-xs text-[#6E675B] mt-0.5 font-medium">
                            依全縣建案實價登錄之成交平均單價排序，呈現宜蘭房市身價最高、行情領先之十大指標建案
                        </p>
                    </div>
                    ...
                    <table class="w-full text-left text-xs sm:text-sm text-[#24211D]">
                        <thead class="bg-[#F2ECE1] text-[#38342D] font-serif-tc font-bold border-b border-[#DCD4C5]">
                            <tr>
                                ...
                                <th class="py-3 px-3.5 whitespace-nowrap">建案名稱</th>
                                <th class="py-3 px-3 whitespace-nowrap">起造建商</th>
                                <th class="py-3 px-3 text-center font-bold text-[#7A5338] whitespace-nowrap">實登平均單價</th>
                                <th class="py-3 px-3 text-center font-bold text-[#1C1B18] whitespace-nowrap">最高成交單價</th>
            ...
            <div class="bg-[#FFFFFF] p-5 sm:p-6 rounded-2xl sm:rounded-3xl border border-[#DCD4C5] shadow-xs">
                <div class="flex items-center justify-between mb-4 flex-wrap gap-2">
                    <div>
                        ...
                        <p class="text-xs text-[#6E675B] mt-0.5 font-medium">
                            依宜蘭縣政府與內政部最新核准申報備查日期排序，精選最新核准之 5 大預售建案，點擊建案卡片可即刻查看完整履歷分區與銷況
                        </p>
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6" id="updateNewProjectsGrid">
                    <!-- Populated dynamically by JavaScript -->
                </div>
            </div>
    ...
    <script>
        ...
        function renderWeeklyUpdateView() {
            ...
            // 2. Render Section 2: Recent 5 Pre-sale Buildcases (固定近期五案 - 響應式雙欄/全幅焦點排版)
            ...
            if (grid) {
                ...
                recentProjects.forEach((p, idx) => {
                    ...
                    const isFirst = idx === 0;
                    const rankLabel = isFirst ? '🏆 最新備查首案' : '最新備查第 ' + (idx + 1) + ' 案';
                    const rankBadgeClass = isFirst
                        ? 'bg-[#FEF3C7] text-[#92400E] border-[#FDE68A] font-bold'
                        : 'bg-[#F4F1EA] text-[#5C564C] border-[#DCD4C5] font-semibold';

                    let salesTag = '';
                    if (hasSales) {
                        salesTag = '<span class="px-2.5 py-0.5 rounded-full text-xs font-mono font-bold bg-[#EEF4EC] text-[#2C4A24] border border-[#BDD9B4]">已售 ' + s.soldUnits + '/' + (p.household || s.totalHouseholds) + '戶 (' + s.salesRate + '%)</span>';
                    } else {
                        salesTag = '<span class="px-2.5 py-0.5 rounded-full text-xs font-medium bg-[#FAF8F5] text-[#7A7366] border border-[#E8E2D5]">尚無成交揭露</span>';
                    }

                    const landTag = (l && l.hasLandData)
                        ? '<span class="inline-flex items-center gap-1 text-[11px] font-bold text-[#2C4A24] bg-[#EEF5EC] px-2 py-0.5 rounded border border-[#BDD9B4]">🌱 購地 ' + l.avgLandPricePerPing + '萬/坪</span>'
                        : '';

                    const card = document.createElement('div');
                    
                    if (isFirst) {
                        // Card 1: Featured Newest Project (Full width across 2 columns on tablet/desktop)
                        card.className = 'md:col-span-2 bg-[#FAF8F5] hover:bg-[#FAF6EF] p-5 sm:p-6 rounded-2xl sm:rounded-3xl border-2 border-[#D8CBB7] hover:border-[#7A5338] shadow-xs hover:shadow-md transition-all duration-200 cursor-pointer flex flex-col justify-between group';
                        card.onclick = () => openDetailModal(p.id);
                        card.innerHTML = 
                            '<div>' +
                                '<!-- Top Badges Row -->' +
                                '<div class="flex items-center justify-between gap-2 flex-wrap pb-3 border-b border-[#EAE4D8]">' +
                                    '<div class="flex items-center gap-2">' +
                                        '<span class="px-2.5 py-1 rounded-lg text-xs font-mono border ' + rankBadgeClass + ' flex items-center gap-1">' +
                                            rankLabel +
                                        '</span>' +
                                        '<span class="px-2.5 py-1 text-xs font-serif-tc font-bold rounded-lg border ' + townColor + '">' +
                                ...
                                '<!-- Responsive Body: Stacked on Mobile, 2-Column Grid on Desktop -->' +
                                '<div class="grid grid-cols-1 lg:grid-cols-12 gap-5 mt-4">' +
                                    '<!-- Left Column: Identity & Builder -->' +
                                    '<div class="lg:col-span-5 flex flex-col justify-between">' +
                                        '<div>' +
                                            '<h4 class="text-lg sm:text-xl md:text-2xl font-serif-tc font-black text-[#1C1B18] group-hover:text-[#7A5338] transition-colors leading-tight mb-2">' +
                                                escapeHtml(p.caseName) +
                                            '</h4>' +
                                            '<div class="text-xs sm:text-sm text-[#5C564C] flex items-center gap-1.5 mb-3 flex-wrap">' +
                                                '<span class="font-medium text-[#7A5338]">起造建商:</span>' +
                                                '<span class="font-semibold text-[#1C1B18]">' + escapeHtml(p.builder || '--') + '</span>' +
                                                (rep ? '<span class="text-xs text-[#7A7366]">' + escapeHtml(rep) + '</span>' : '') +
                                            '</div>' +
                                            '<p class="text-xs text-[#7A7366] leading-relaxed hidden sm:block">' +
                                                '本案為近期全縣最新核准申報備查之指標建案，座落於' + escapeHtml(p.town || '') + '，規劃總戶數 ' + (p.household || '--') + ' 戶。' +
                                            '</p>' +
                                        '</div>' +
                                    '</div>' +

                                    '<!-- Right Column: 4 Stat Tiles & Location/Permit -->' +
                                    '<div class="lg:col-span-7 space-y-3">' +
                                        '<!-- 4 Key Stat Tiles -->' +
                                        '<div class="grid grid-cols-2 sm:grid-cols-4 gap-2.5">' +
                                            '<div class="bg-[#FFFFFF] p-3 rounded-xl border border-[#EBE5DA] shadow-2xs">' +
                                                '<div class="text-[11px] text-[#7A7366] font-medium mb-1 flex items-center gap-1">' +
                                                    '<span>📋</span> 申報備查日' +
                                                '</div>' +
                                                '<div class="font-mono font-black text-xs sm:text-sm text-[#1C1B18]">' +
                                            ...
                                            '<div class="bg-[#FFFFFF] p-3 rounded-xl border border-[#EBE5DA] shadow-2xs">' +
                                                '<div class="text-[11px] text-[#7A7366] font-medium mb-1 flex items-center gap-1">' +
                                                    '<span>🏢</span> 規劃總戶數' +
                                                '</div>' +
                                                '<div class="font-mono font-black text-xs sm:text-sm text-[#7A5338]">' +
                                                    (p.household || '--') + ' <span class="text-[10.5px] font-normal text-[#7A7366]">戶</span>' +
                                                '</div>' +
                                            '</div>' +
                                            '<div class="bg-[#FFFFFF] p-3 rounded-xl border border-[#EBE5DA] shadow-2xs">' +
                                                '<div class="text-[11px] text-[#7A7366] font-medium mb-1 flex items-center gap-1">' +
                                                    '<span>📅</span> 建照核准日' +
                                                '</div>' +
                                                '<div class="font-mono font-bold text-xs sm:text-sm text-[#38342D]">' +
                                            ...
                                            '<div class="bg-[#FFFFFF] p-3 rounded-xl border border-[#EBE5DA] shadow-2xs">' +
                                                '<div class="text-[11px] text-[#7A7366] font-medium mb-1 flex items-center gap-1">' +
                                                    '<span>📈</span> 實登銷售率' +
                                                '</div>' +
                                                '<div class="font-mono font-bold text-xs sm:text-sm ' + (hasSales ? 'text-[#2C4A24]' : 'text-[#7A7366]') + '">' +
                                                    (hasSales ? s.salesRate + '%' : '尚未揭露') +
                                                '</div>' +
                                            '</div>' +
                                        '</div>' +

                                        '<!-- Location & Permit details (Spacious Full-Width Box) -->' +
                                        '<div class="bg-[#FFFFFF] p-3 rounded-xl border border-[#EBE5DA] text-xs space-y-1.5">' +
                                            '<div class="flex items-start gap-2">' +
                                                '<span class="text-[#7A7366] shrink-0 font-medium flex items-center gap-1">' +
                                                    '<span>📍</span> 基地座落:' +
                                                '</span>' +
                                                '<span class="font-medium text-[#1C1B18] break-words">' +
                                                    escapeHtml(p.mainLand || '尚未載明') +
                                                '</span>' +
                                            '</div>' +
                                            '<div class="flex items-start sm:items-center justify-between gap-2 pt-1.5 border-t border-[#F0EAE1] flex-col sm:flex-row">' +
                                                '<div class="flex items-center gap-1.5 text-[#7A7366]">' +
                                                    '<span>📜 建照字號:</span>' +
                                                    '<span class="font-mono text-[#38342D]">' + escapeHtml(p.permitNo || '--') + '</span>' +
                                                '</div>' +
                                            '</div>' +
                                        '</div>' +
                                    '</div>' +
                            ...
                            '<!-- Footer Action Row -->' +
                            '<div class="flex items-center justify-between pt-3 mt-4 border-t border-[#EAE4D8] text-xs">' +
                                '<span class="text-[#8C8477]">點擊卡片查看完整建案履歷與土地分區</span>' +
                                '<span class="font-bold text-[#7A5338] group-hover:text-[#4A2F1C] flex items-center gap-1 bg-[#F7EFE8] px-3.5 py-1.5 rounded-xl border border-[#DECDBE] group-hover:bg-[#EFE5DA] transition">' +
                                    '查看建案完整履歷 ➔' +
                                '</span>' +
                            '</div>';
                    } else {
                        // Cards 2 to 5: Standard Grid Cards (1 column each on md/lg)
                        card.className = 'md:col-span-1 bg-[#FAF8F5] hover:bg-[#FAF6EF] p-5 rounded-2xl border border-[#E8E2D5] hover:border-[#4A5D44] shadow-xs hover:shadow-md transition-all duration-200 cursor-pointer flex flex-col justify-between group';
                        card.onclick = () => openDetailModal(p.id);
                        card.innerHTML = 
                            '<div>' +
                                '<!-- Top Header Row -->' +
                                '<div class="flex items-center justify-between gap-2 flex-wrap mb-2.5">' +
                                    '<div class="flex items-center gap-2">' +
                                        '<span class="px-2.5 py-0.5 rounded-lg text-xs font-mono border ' + rankBadgeClass + '">' +
                                            rankLabel +
                                        '</span>' +
                                        '<span class="px-2.5 py-0.5 text-xs font-serif-tc font-bold rounded-lg border ' + townColor + '">' +
                                            escapeHtml(p.town || '宜蘭縣') +
                                        '</span>' +
                                    '</div>' +
                                    salesTag +
                                ...
                                '<!-- Project Name -->' +
                                '<h4 class="text-base sm:text-lg font-serif-tc font-black text-[#1C1B18] group-hover:text-[#4A5D44] transition-colors leading-snug mb-1.5 line-clamp-1">' +
                                    escapeHtml(p.caseName) +
                                '</h4>' +

                                '<!-- Builder & Rep -->' +
                                '<div class="text-xs text-[#5C564C] flex items-center gap-1.5 mb-3.5 flex-wrap">' +
                                    '<span class="font-medium text-[#7A5338]">起造:</span>' +
                                    '<span class="font-semibold text-[#1C1B18]">' + escapeHtml(p.builder || '--') + '</span>' +
                                    (rep ? '<span class="text-xs text-[#7A7366]">' + escapeHtml(rep) + '</span>' : '') +
                                    landTag +
                                '</div>' +

                                '<!-- 2x2 Key Figures Grid (Spacious tiles, zero cramping) -->' +
                                '<div class="grid grid-cols-2 gap-2.5 mb-3">' +
                                    '<div class="bg-[#FFFFFF] p-2.5 sm:p-3 rounded-xl border border-[#EBE5DA] shadow-2xs">' +
                                        '<div class="text-[11px] text-[#7A7366] font-medium mb-1 flex items-center gap-1">' +
                                            '<span>📋</span> 申報備查日' +
                                        '</div>' +
                                        '<div class="font-mono font-black text-xs sm:text-sm text-[#1C1B18]">' +
                                ...
                                '<!-- Location & Permit details box (Full width) -->' +
                                '<div class="bg-[#FFFFFF] p-3 rounded-xl border border-[#EBE5DA] text-xs space-y-1.5">' +
                                    '<div class="flex items-start gap-2">' +
                                        '<span class="text-[#7A7366] shrink-0 font-medium flex items-center gap-1">' +
                                            '<span>📍</span> 基地:' +
                                        '</span>' +
                                        '<span class="font-medium text-[#1C1B18] break-words line-clamp-1 group-hover:line-clamp-none transition-all">' +
                                            escapeHtml(p.mainLand || '尚未載明') +
                                        '</span>' +
                                    '</div>' +
                                    '<div class="flex items-center gap-1.5 pt-1.5 border-t border-[#F0EAE1] text-[#7A7366]">' +
                                        '<span>📜 建照:</span>' +
                                        '<span class="font-mono text-[#38342D] truncate">' + escapeHtml(p.permitNo || '--') + '</span>' +
                                    '</div>' +
                                '</div>' +
                            '</div>' +

                            '<!-- Footer Action Row -->' +
                            '<div class="flex items-center justify-between pt-3 mt-3 border-t border-[#EAE4D8] text-xs">' +
                                '<span class="text-[11px] text-[#8C8477]">詳情與分區</span>' +
                                '<span class="font-bold text-[#4A5D44] group-hover:text-[#2C4A24] flex items-center gap-1 bg-[#EEF4EC] px-3 py-1 rounded-lg border border-[#C5D9C0] group-hover:bg-[#E0ECD9] transition">' +
                                    '建案詳情 ➔' +
                                '</span>' +
                            '</div>';
                    }
            ...
            // 3. Render Section 3: Recent 30 Transactions (固定近期30筆)
            const allTx = [];
            allProjects.forEach(p => {
                const s = p.salesStats || {};
        ...
        function renderTopPriceProjects(targetTableId) {
            ...
            const candidates = allProjects.filter(p => p.salesStats && p.salesStats.avgPricePerPing > 0);
            candidates.sort((a, b) => b.salesStats.avgPricePerPing - a.salesStats.avgPricePerPing);
            ...
            top10.forEach((p, idx) => {
                ...
                const rep = (bInfo?.representative || bInfo?.rep) ? \`<span class="text-[10.5px] font-serif-tc font-bold text-[#7A5338] bg-[#F7EFE8] px-1.5 py-0.2 rounded border border-[#DECDBE] ml-1">\${bInfo.representative || bInfo.rep}</span>\` : '';
                ...
                const priceRangeStr = (s.minPricePerPing && s.maxPricePerPing && s.minPricePerPing !== s.maxPricePerPing)
                    ...
                    : (s.avgPricePerPing ? \`\${s.avgPricePerPing} 萬/坪\` : '--');
                ...
                tr.innerHTML = \`
                    ...
                    <td class="py-3 px-3 text-center whitespace-nowrap">
                        <span class="font-serif-tc font-black text-sm sm:text-base text-[#7A5338]">\${s.avgPricePerPing ? s.avgPricePerPing + ' 萬/坪' : '--'}</span>
                    </td>
                    <td class="py-3 px-3 text-center whitespace-nowrap">
                        <span class="font-mono font-bold text-xs sm:text-sm text-[#1C1B18]">\${s.maxPricePerPing ? s.maxPricePerPing + ' 萬/坪' : '--'}</span>
                    </td>
                    ...
                    <td class="py-3 px-3 text-center whitespace-nowrap">
                        ...
                        <div class="text-[10.5px] font-bold \${s.isSoldOut ? 'text-[#2C4A24]' : 'text-[#4A5D44]'}">
                            \${s.isSoldOut ? '完銷 100%' : \`\${s.salesRate}%\`}
                        </div>
                    </td>
                    <td class="py-3 px-3 text-center whitespace-nowrap">
