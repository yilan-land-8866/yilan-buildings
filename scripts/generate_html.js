...
<body class="antialiased min-h-screen flex flex-col selection:bg-[#DCD3C3] selection:text-[#1C1B18]">
    ...
    <header class="bg-[#FFFFFF]/95 backdrop-blur-md border-b border-[#DCD4C5] sticky top-0 z-30 shadow-xs">
        <div class="fluid-container">
            <!-- Mobile: 2 rows (Row 1: Title/Badge/Export, Row 2: 4 Tabs) | Desktop: 1 row -->
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between py-2 sm:py-2.5 gap-2 sm:gap-4 sm:min-h-[4.5rem]">
                
                <!-- Row 1 on mobile / Left on desktop -->
                <div class="flex items-center justify-between min-w-0 flex-1">
                    <div class="min-w-0">
                        <div class="flex items-center gap-2 flex-wrap">
                            <h1 class="text-base sm:text-2xl md:text-3xl font-serif-tc font-black tracking-tight sm:tracking-wide text-[#1C1B18] leading-tight truncate">
                                宜蘭縣建案備查與實價登錄檢索
                            </h1>
                    ...
                    <button onclick="exportCurrentViewToExcel()" title="匯出 Excel" class="sm:hidden ml-2 p-1.5 text-xs font-semibold rounded-lg bg-[#FFFFFF] text-[#3E523A] border border-[#C5D4C2] hover:bg-[#EEF4EC] shrink-0 shadow-xs flex items-center justify-center">
                ...
                <!-- Row 2 on mobile (4 columns grid) / Right on desktop (Flex row) -->
                <div class="grid grid-cols-4 sm:flex sm:items-center gap-1 sm:gap-2 shrink-0 w-full sm:w-auto">
                    <!-- 1. 建案檢索 Tab -->
                    <button id="tabSearchBtn" onclick="switchView('search')" class="px-2 sm:px-4 py-1.5 sm:py-2.5 text-xs sm:text-sm font-semibold rounded-lg sm:rounded-xl bg-[#1C1B18] text-[#FAF8F5] flex items-center justify-center gap-1 sm:gap-1.5 transition shadow-xs">
                    ...
                    <!-- 2. 建商名冊 Tab -->
                    <button id="tabBuildersBtn" onclick="switchView('builders')" class="px-2 sm:px-4 py-1.5 sm:py-2.5 text-xs sm:text-sm font-semibold rounded-lg sm:rounded-xl text-[#38342D] hover:bg-[#EAE4D8] border border-[#DCD4C5] bg-[#FFFFFF] flex items-center justify-center gap-1 sm:gap-1.5 transition">
                    ...
                    <!-- 3. 行情統計 Tab -->
                    <button id="tabStatsBtn" onclick="switchView('stats')" class="px-2 sm:px-4 py-1.5 sm:py-2.5 text-xs sm:text-sm font-semibold rounded-lg sm:rounded-xl text-[#38342D] hover:bg-[#EAE4D8] border border-[#DCD4C5] bg-[#FFFFFF] flex items-center justify-center gap-1 sm:gap-1.5 transition">
                    ...
                    <!-- 4. 排行榜 Tab -->
                    <button id="tabUpdateBtn" onclick="switchView('update')" class="px-2 sm:px-4 py-1.5 sm:py-2.5 text-xs sm:text-sm font-semibold rounded-lg sm:rounded-xl text-[#38342D] hover:bg-[#EAE4D8] border border-[#DCD4C5] bg-[#FFFFFF] flex items-center justify-center gap-1 sm:gap-1.5 transition">
                        ...
                        <span>排行榜</span>
                        <span class="px-1 py-0.2 rounded-full text-[9px] font-mono font-bold bg-[#FEF3C7] text-[#92400E] ml-0.5 border border-[#FDE68A]">TOP</span>
                    </button>

                    <!-- Desktop Export Excel Button -->
                    <button onclick="exportCurrentViewToExcel()" title="匯出 Excel" class="hidden sm:flex px-2.5 sm:px-3.5 py-1.5 sm:py-2.5 text-xs sm:text-sm font-semibold rounded-lg sm:rounded-xl bg-[#FFFFFF] text-[#3E523A] border border-[#C5D4C2] hover:bg-[#EEF4EC] items-center justify-center gap-1 sm:gap-1.5 transition shadow-xs">
    ...
    <main class="fluid-container py-6 flex-1 w-full">
        ...
        <section id="kpiMetricsSection" class="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4.5 mb-6">
        ...
        <div id="statsView" class="hidden space-y-8">
            ...
            <div class="bg-[#FFFFFF] p-5 sm:p-6 rounded-2xl sm:rounded-3xl border border-[#DCD4C5] shadow-xs">
                <div class="flex items-center justify-between mb-4 flex-wrap gap-2">
                    ...
                    <div class="flex items-center gap-2">
                        <span class="sm:hidden text-[11px] font-medium text-[#8C6D2B] bg-[#FAF3EB] px-2.5 py-1 rounded-lg border border-[#DECDBE]">
                            👈 左右滑動表格 👉
                        </span>
                ...
                <div class="overflow-x-auto rounded-xl border border-[#EBE5DA] shadow-xs">
                    <table class="w-full min-w-[760px] text-left text-xs sm:text-sm text-[#24211D]">
                        <thead class="bg-[#F2ECE1] text-[#38342D] font-serif-tc font-bold border-b border-[#DCD4C5]">
                            <tr>
                                <th class="py-3 px-3.5 whitespace-nowrap min-w-[80px]">行政區</th>
                                <th class="py-3 px-3 text-center whitespace-nowrap min-w-[90px]">樣本數</th>
                                <th class="py-3 px-3 text-right whitespace-nowrap min-w-[105px] text-[#8C6D2B]">基地平均單價</th>
                                <th class="py-3 px-3 text-right whitespace-nowrap min-w-[105px] font-bold text-[#1C1B18]">建案成交均價</th>
                                <th class="py-3 px-3 text-center whitespace-nowrap min-w-[90px]">房地倍數比</th>
                                <th class="py-3 px-3 text-center whitespace-nowrap min-w-[95px]">平均開發時程</th>
                                <th class="py-3 px-4 whitespace-nowrap min-w-[220px]">區域市場特性解析</th>
                            </tr>
                        </thead>
            ...
            <div class="bg-[#FFFFFF] p-5 sm:p-6 rounded-2xl sm:rounded-3xl border border-[#DCD4C5] shadow-xs space-y-4">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        <div class="flex bg-[#F2ECE1] p-0.5 sm:p-1 rounded-xl text-xs font-semibold border border-[#DCD4C5]">
                            <button id="rBtnStatusAll" onclick="filterResearchStatus('all')" class="px-2.5 sm:px-3 py-1.5 rounded-lg bg-[#FFFFFF] text-[#1C1B18] shadow-xs transition text-[11px] sm:text-xs">全部 (367)</button>
                            <button id="rBtnStatusSold" onclick="filterResearchStatus('sold')" class="px-2.5 sm:px-3 py-1.5 rounded-lg text-[#6E675B] hover:text-[#1C1B18] transition text-[11px] sm:text-xs">已開賣 (295)</button>
                            <button id="rBtnStatusReserve" onclick="filterResearchStatus('reserve')" class="px-2.5 sm:px-3 py-1.5 rounded-lg text-[#6E675B] hover:text-[#1C1B18] transition text-[11px] sm:text-xs">儲備中 (72)</button>
                        </div>

                        <!-- Town Filter -->
                        <select id="rTownFilter" onchange="applyResearchFilters()" class="text-xs bg-[#FAF8F5] border border-[#D0C7B8] rounded-xl px-2.5 py-2 text-[#1C1B18] font-medium focus:bg-[#FFFFFF] focus:outline-none">
                            <option value="">全部鄉鎮</option>
                        </select>

                        <!-- Search Input -->
                        <div class="relative flex-1 min-w-[170px] sm:flex-initial">
                            <input type="text" id="rSearchInput" oninput="applyResearchFilters()" placeholder="搜尋建案、建商、地號、路名..." class="text-xs bg-[#FAF8F5] border border-[#D0C7B8] rounded-xl pl-8 pr-3 py-2 w-full sm:w-64 text-[#1C1B18] placeholder-[#9E9689] focus:bg-[#FFFFFF] focus:outline-none">
                ...
                <!-- Table Container -->
                <div class="overflow-x-auto rounded-xl border border-[#EBE5DA] max-h-[640px] overflow-y-auto shadow-xs">
                    <table class="w-full min-w-[1180px] text-left text-xs sm:text-sm text-[#24211D]">
            ...
            <div class="bg-[#FFFFFF] p-5 sm:p-6 rounded-2xl sm:rounded-3xl border border-[#DCD4C5] shadow-xs">
                <div class="flex items-center justify-between mb-4 flex-wrap gap-2">
                    <div>
                        ...
                        <p class="text-xs text-[#6E675B] mt-0.5 font-medium">
                            依全縣建案實價登錄之成交單價中位數排序（排除極端高低樓層偏差），並詳列成交單價區間、成交總價區間與成交時間
                        </p>
                    </div>
                    <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-mono font-bold bg-[#FAF5EB] text-[#7A5338] border border-[#E5D7B3]">
                        <span class="w-1.5 h-1.5 rounded-full bg-[#8C6D2B] animate-pulse"></span>
                        單價中位數 Top 10
                    </span>
                </div>
                    ...
                    <table class="w-full text-left text-xs sm:text-sm text-[#24211D]">
                        <thead class="bg-[#F2ECE1] text-[#38342D] font-serif-tc font-bold border-b border-[#DCD4C5]">
                            <tr>
                                ...
                                <th class="py-3 px-3.5 whitespace-nowrap">建案名稱</th>
                                <th class="py-3 px-3 whitespace-nowrap">起造建商</th>
                                <th class="py-3 px-3 text-center whitespace-nowrap">成交時間</th>
                                <th class="py-3 px-3 text-center font-bold text-[#7A5338] whitespace-nowrap">單價中位數 (排名)</th>
                                <th class="py-3 px-3 text-center whitespace-nowrap">單價成交區間</th>
                                <th class="py-3 px-3 text-center font-bold text-[#1C1B18] whitespace-nowrap">成交總價區間</th>
                                <th class="py-3 px-3 text-center whitespace-nowrap">累計銷售進度</th>
                                <th class="py-3 px-3 text-center font-bold text-[#2C4A24] whitespace-nowrap">🌱 基地購地地價</th>
                                <th class="py-3 px-3 text-center whitespace-nowrap">詳情</th>
                            </tr>
                        </thead>
                        <tbody id="updateTopPriceProjectsTableBody" class="divide-y divide-[#EBE5DA]">
                            <!-- Populated dynamically by JavaScript -->
                        </tbody>
                    </table>
                </div>
            ...
            <div class="bg-[#FFFFFF] p-5 sm:p-6 rounded-2xl sm:rounded-3xl border border-[#DCD4C5] shadow-xs">
                <div class="flex items-center justify-between mb-4 flex-wrap gap-2">
                    <div>
                        ...
                        <p class="text-xs text-[#6E675B] mt-0.5 font-medium">
                            依宜蘭縣政府與內政部最新核准申報備查日期排序，精選最新核准申報之 5 大指標建案（依序五名）
                        </p>
                    </div>
                    ...
                    <table class="w-full text-left text-xs sm:text-sm text-[#24211D]">
                        <thead class="bg-[#F2ECE1] text-[#38342D] font-serif-tc font-bold border-b border-[#DCD4C5]">
                            <tr>
                                ...
                                <th class="py-3 px-3.5 whitespace-nowrap">建案名稱</th>
                                <th class="py-3 px-3 whitespace-nowrap">起造建商</th>
                                <th class="py-3 px-3 text-center font-bold text-[#1C1B18] whitespace-nowrap">申報備查日</th>
                                <th class="py-3 px-3 text-center whitespace-nowrap">建照核准日</th>
                                <th class="py-3 px-3 text-center font-bold text-[#7A5338] whitespace-nowrap">規劃總戶數</th>
                                <th class="py-3 px-3 whitespace-nowrap">建造執照字號</th>
                                <th class="py-3 px-3 whitespace-nowrap">基地座落地號</th>
                                <th class="py-3 px-3 text-center whitespace-nowrap">實登去化進度</th>
                                <th class="py-3 px-3 text-center font-bold text-[#2C4A24] whitespace-nowrap">🌱 基地購地地價</th>
                                <th class="py-3 px-3 text-center whitespace-nowrap">詳情</th>
                            </tr>
                        </thead>
                        <tbody id="updateNewProjectsTableBody" class="divide-y divide-[#EBE5DA]">
                            <!-- Populated dynamically by JavaScript -->
                        </tbody>
                    </table>
                </div>
            ...
            <div class="bg-[#FFFFFF] p-5 sm:p-6 rounded-2xl sm:rounded-3xl border border-[#DCD4C5] shadow-xs">
                <div class="flex items-center justify-between mb-4 flex-wrap gap-2">
                    <div>
                        ...
                        <p class="text-xs text-[#6E675B] mt-0.5 font-medium">
                            內政部最新揭露之預售屋買賣交易明細，依成交日期遞減排序精選最新 30 筆
                        </p>
                    </div>
                    <div class="flex items-center gap-2">
    ...
    <script>
        ...
        function switchView(view) {
            ...
            const kpiSection = document.getElementById('kpiMetricsSection');
            ...
            [searchBtn, buildersBtn, statsBtn, updateBtn].forEach(btn => {
                if (btn) btn.className = 'px-2 sm:px-4 py-1.5 sm:py-2.5 text-xs sm:text-sm font-semibold rounded-lg sm:rounded-xl text-[#38342D] hover:bg-[#EAE4D8] border border-[#DCD4C5] bg-[#FFFFFF] flex items-center justify-center gap-1 sm:gap-1.5 transition';
            });
            ...
            if (view === 'update') {
                ...
                if (updateBtn) updateBtn.className = 'px-2 sm:px-4 py-1.5 sm:py-2.5 text-xs sm:text-sm font-semibold rounded-lg sm:rounded-xl bg-[#1C1B18] text-[#FAF8F5] flex items-center justify-center gap-1 sm:gap-1.5 transition shadow-xs';
                renderWeeklyUpdateView();
            } else {
                if (kpiSection) kpiSection.classList.remove('hidden');
                if (view === 'search') {
                    if (searchView) searchView.classList.remove('hidden');
                    if (searchBtn) searchBtn.className = 'px-2 sm:px-4 py-1.5 sm:py-2.5 text-xs sm:text-sm font-semibold rounded-lg sm:rounded-xl bg-[#1C1B18] text-[#FAF8F5] flex items-center justify-center gap-1 sm:gap-1.5 transition shadow-xs';
        ...
        function renderTopHotSellingProjects(targetTableId) {
            ...
            allProjects.forEach(p => {
                ...
                const recentTx = s.transactions.filter(t => {
                    const isPark = t.isParking || t.layout === '獨立車位' || t.source === '預售車位';
            ...
            if (matrixBody && window.TOWNSHIP_MATRIX) {
                matrixBody.innerHTML = window.TOWNSHIP_MATRIX.map(t => {
                    return '<tr class="hover:bg-[#FAF8F5] transition text-xs sm:text-sm">' +
                        '<td class="py-3 px-3.5 font-bold text-[#1C1B18] whitespace-nowrap">' + t.town + '</td>' +
                        '<td class="py-3 px-3 text-center whitespace-nowrap"><span class="inline-block whitespace-nowrap px-2.5 py-0.5 rounded-full text-xs font-semibold bg-[#EBF5FB] text-[#2874A6] border border-[#AED6F1]">' + t.count + ' 案</span></td>' +
                        '<td class="py-3 px-3 text-right font-medium text-[#8C6D2B] font-mono whitespace-nowrap">' + t.avgLand + ' 萬/坪</td>' +
                        '<td class="py-3 px-3 text-right font-bold text-[#1C1B18] font-mono whitespace-nowrap">' + t.avgSales + ' 萬/坪</td>' +
                        '<td class="py-3 px-3 text-center font-bold text-[#7A5338] font-mono whitespace-nowrap">' + t.avgRatio + ' 倍</td>' +
                        '<td class="py-3 px-3 text-center text-[#6E675B] font-mono whitespace-nowrap">' + (t.avgMonths ? t.avgMonths + ' 個月' : '--') + '</td>' +
                        '<td class="py-3 px-4 text-xs text-[#5C564C] min-w-[200px] leading-relaxed">' + (t.notes || '--') + '</td>' +
                    '</tr>';
                }).join('');
            }
        ...
        function openDetailModal(id) {
            const p = allProjects.find(item => String(item.id) === String(id) || item.id === id);
            if (!p) return;
            ...
            const townElem = document.getElementById('modalTownBadge');
            if (townElem) townElem.innerText = p.town || '宜蘭縣';
            const planElem = document.getElementById('modalPlanBadge');
            if (planElem) planElem.innerText = z.planTypeShort || '土地分區';
            const numElem = document.getElementById('modalNumberCode');
            if (numElem) numElem.innerText = p.numberCode ? ('備查編號: ' + p.numberCode) : '';
            const caseElem = document.getElementById('modalCaseName');
            if (caseElem) caseElem.innerText = p.caseName || '未命名建案';
            const builderHdr = document.getElementById('modalBuilderHeader');
            if (builderHdr) builderHdr.innerText = '起造人：' + (p.builder || '未提供');
            ...
            if (hasSales) {
                ...
                if (statusBadge) {
                    if (s.salesRate >= 90) {
                        ...
                        statusBadge.innerText = s.salesRate >= 100 ? '已完銷 100%' : '熱銷完銷倒數';
                        if (progBar) progBar.className = 'h-full bg-[#4A5D44] progress-bar-fill rounded-full';
                    } else {
                        ...
                        if (progBar) progBar.className = 'h-full bg-[#7A5338] progress-bar-fill rounded-full';
                    }
                }

                const avgP = document.getElementById('modalAvgPricePing');
                if (avgP) avgP.innerText = (s.avgPricePerPing ? s.avgPricePerPing + ' 萬/坪' : '--');
                const rangeP = document.getElementById('modalPriceRangePing');
                if (rangeP) rangeP.innerText = (s.minPricePerPing && s.maxPricePerPing) ? (s.minPricePerPing + ' ~ ' + s.maxPricePerPing + ' 萬/坪') : '--';
                const totalP = document.getElementById('modalAvgTotalPrice');
                if (totalP) totalP.innerText = s.avgTotalPrice ? (Math.round(s.avgTotalPrice).toLocaleString() + ' 萬元') : '--';
                const dateP = document.getElementById('modalLatestTransDate');
                if (dateP) dateP.innerText = s.latestTransactionDate || '--';
            } else {
                const sCount = document.getElementById('modalSalesCountText');
                if (sCount) sCount.innerText = '實登已售 0 / ' + (p.household || s.totalHouseholds || '0') + ' 戶';
                const sRate = document.getElementById('modalSalesRateText');
                if (sRate) sRate.innerText = '0%';
                if (progBar) {
                ...
                const avgP = document.getElementById('modalAvgPricePing');
                if (avgP) avgP.innerText = '--';
                const rangeP = document.getElementById('modalPriceRangePing');
                if (rangeP) rangeP.innerText = '--';
                const totalP = document.getElementById('modalAvgTotalPrice');
                if (totalP) totalP.innerText = '--';
                const dateP = document.getElementById('modalLatestTransDate');
                if (dateP) dateP.innerText = '尚無成交';
            }
            ...
            if (txBody) {
                txBody.innerHTML = '';
                const txList = s.transactions || [];
                const txCountBadge = document.getElementById('modalTransCountBadge');
            ...
            if (landBody) {
                landBody.innerHTML = '';
                const landBadge = document.getElementById('modalLandTxCountBadge');
                const avgLandP = document.getElementById('modalAvgLandPrice');
                const totalLandC = document.getElementById('modalTotalLandCost');
                const totalLandA = document.getElementById('modalTotalLandArea');
                const latestLandD = document.getElementById('modalLatestLandDate');

                if (l.hasLandData && l.txCount > 0) {
                    if (landBadge) landBadge.innerText = l.txCount + ' 筆購地交易';
                    if (avgLandP) avgLandP.innerText = l.avgLandPricePerPing + ' 萬/坪';
                    if (totalLandC) totalLandC.innerText = l.totalLandCostWan ? (l.totalLandCostWan.toLocaleString() + ' 萬元') : '--';
                    if (totalLandA) totalLandA.innerText = l.totalLandAreaPing ? (l.totalLandAreaPing.toLocaleString() + ' 坪') : '--';
                    if (latestLandD) latestLandD.innerText = l.latestLandDate || '--';
                    ...
                    if (landBadge) landBadge.innerText = '無購地揭露';
                    if (avgLandP) avgLandP.innerText = '--';
                    if (totalLandC) totalLandC.innerText = '--';
                    if (totalLandA) totalLandA.innerText = '--';
                    if (latestLandD) latestLandD.innerText = '無紀錄';
                    landBody.innerHTML = '<tr><td colspan="6" class="text-center py-4 text-[#7A7366]">實價登錄暫無此基地在建照核發前之土地獨立買賣揭露紀錄（可能為地主合建、自地自建或早期取得）</td></tr>';
                }
            }

            // Builder Info
            const bRep = document.getElementById('modalBuilderRep');
            const bUni = document.getElementById('modalBuilderUni');
            const bCap = document.getElementById('modalBuilderCap');
            const bAddr = document.getElementById('modalBuilderAddr');
            if (bInfo) {
                if (bRep) bRep.innerText = bInfo.representative || '自然人/未提供';
                if (bUni) bUni.innerText = (bInfo.unifiedNo || '無統編') + ' (' + (bInfo.status || '核准設立') + ')';
                if (bCap) bCap.innerText = bInfo.capital || '未提供';
                if (bAddr) bAddr.innerText = bInfo.address || '未提供公司地址';
            } else {
                if (bRep) bRep.innerText = p.builder || '未提供';
                if (bUni) bUni.innerText = '未建立工商登記';
                if (bCap) bCap.innerText = '未提供';
                if (bAddr) bAddr.innerText = '未提供';
            }
            ...
            const mUse = document.getElementById('modalMainUse');
            if (mUse) mUse.innerText = p.normalizedUse || '住宅';
            const mMat = document.getElementById('modalMaterial');
            if (mMat) mMat.innerText = p.normalizedMaterial || 'RC';
            const mPDate = document.getElementById('modalPermitDate');
            if (mPDate) mPDate.innerText = p.permitDate || '未填寫';
            const mDDate = document.getElementById('modalDeclareDate');
            if (mDDate) mDDate.innerText = p.declareDate || '未填寫';
            const mFDate = document.getElementById('modalFirstRegDate');
            if (mFDate) mFDate.innerText = p.firstRegDate || '尚未登記';
            
            const mStreet = document.getElementById('modalStreetLocation');
            if (mStreet) mStreet.innerText = '坐落位置：' + (p.town || '') + ' ' + (p.location || '無街道路名');
            const mLand = document.getElementById('modalMainLand');
            if (mLand) mLand.innerText = '基地地號：' + (p.mainLand || '無');
        ...
        function exportCurrentViewToExcel() {
            ...
            try {
                ...
                if (currentView === 'update') {
                    ...
                    if (headerBadge) {
                        const m = headerBadge.textContent.match(/(\d{4})[\/\-](\d{1,2})[\/\-](\d{1,2})/);
                        if (m) baseUpdateDate = new Date(parseInt(m[1]), parseInt(m[2]) - 1, parseInt(m[3]));
                    }
                    const cutoffDate = new Date(baseUpdateDate.getFullYear(), baseUpdateDate.getMonth() - 6, baseUpdateDate.getDate());
                    ...
                    allProjects.forEach(p => {
                        ...
                        if (recentTx.length > 0) {
                            hotCandidates.push({
                                ...
                                salesRate: s.salesRate,
                                avgPrice: s.avgPricePerPing,
                                avgLandPrice: p.landStats?.avgLandPricePerPing || '--'
                            });
                        }
                    });
                    ...
                    const topHotRows = hotCandidates.slice(0, 10).map((item, idx) => ({
                        '排名': idx + 1,
                        '鄉鎮市': item.project.town,
                        '建案名稱': item.project.caseName,
                        '起造建商': item.project.builder,
                        '最新成交日': item.latestDate,
                        '近6個月去化戶數(本榜排名依據)': item.recentCount,
                        '全案累計已售(戶)': item.soldUnits,
                        '全案規劃總戶數': item.totalHouseholds,
                        '全案銷售率(%)': item.salesRate + '%',
                        '實登均價(萬/坪)': item.avgPrice || '--',
                        '基地購地地價(萬/坪)': item.avgLandPrice
                    }));
                    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(topHotRows), '十大熱銷建案榜');

                    const priceCandidates = allProjects.filter(p => p.salesStats && (p.salesStats.medianPricePerPing > 0 || p.salesStats.avgPricePerPing > 0));
                    priceCandidates.sort((a, b) => {
                        const mB = b.salesStats.medianPricePerPing || b.salesStats.avgPricePerPing || 0;
                        const mA = a.salesStats.medianPricePerPing || a.salesStats.avgPricePerPing || 0;
                        if (mB !== mA) return mB - mA;
                        return (b.salesStats.avgPricePerPing || 0) - (a.salesStats.avgPricePerPing || 0);
                    });
                    const topPriceRows = priceCandidates.slice(0, 10).map((p, idx) => ({
                        '排名': idx + 1,
                        '鄉鎮市': p.town,
                        '建案名稱': p.caseName,
                        '起造建商': p.builder,
                        '單價中位數(萬/坪)': p.salesStats.medianPricePerPing || p.salesStats.avgPricePerPing || '--',
                        '單價區間(萬/坪)': (p.salesStats.minPricePerPing && p.salesStats.maxPricePerPing) ? (p.salesStats.minPricePerPing + ' ~ ' + p.salesStats.maxPricePerPing) : '--',
                        '成交均價(萬/坪)': p.salesStats.avgPricePerPing || '--',
                        '全案規劃戶數': p.household || p.salesStats.totalHouseholds,
                        '全案銷售率(%)': p.salesStats.salesRate + '%'
                    }));
                    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(topPriceRows), '成交單價前十名建案');

                    const sortedByDeclare = [...allProjects].filter(p => p.declareDateRaw || p.declareDate);
                    sortedByDeclare.sort((a, b) => String(b.declareDateRaw || b.declareDate || '').localeCompare(String(a.declareDateRaw || a.declareDate || '')));
                    const top5 = sortedByDeclare.slice(0, 5).map(p => ({
                        '鄉鎮市': p.town,
                        '建案名稱': p.caseName,
                        '建設公司': p.builder,
                        '申報備查日': p.declareDate,
                    ...
                    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(top5), '最新備查五案');

                    const allTx = [];
                    allProjects.forEach(p => (p.salesStats?.transactions || []).forEach(t => allTx.push({
                        '成交日期': t.dateRoc,
                        '鄉鎮市': p.town,
                        '建案名稱': p.caseName,
                        '起造建商': p.builder,
                        '交易戶別/樓層': t.unit,
                        '面積(坪)': t.areaPing,
                        '成交單價(萬/坪)': t.pricePerPing,
                        '成交總價(萬)': t.totalPrice,
                        '格局': t.layout,
                        '車位': t.parking
                    })));
                    allTx.sort((a, b) => String(b['成交日期'] || '').localeCompare(String(a['成交日期'] || '')));
                    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(allTx.slice(0, 30)), '最新成交30筆');

                    XLSX.writeFile(wb, '宜蘭建案_房市排行榜.xlsx');
                } else if (currentView === 'builders') {
                    const rows = Object.entries(buildersMap).map(([name, info]) => ({
                        '建商名稱': name,
                        '統一編號': info.unifiedNo || '--',
                        '代表人/負責人': info.representative || '--',
                        '公司地址': info.address || '--',
                        '資本額': info.capital || '--',
                        '推案量(案)': info.projectCount || 0
                    }));
                    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(rows), '建商名冊');
                    XLSX.writeFile(wb, '宜蘭建商與負責人名冊.xlsx');
                } else {
                    const rows = filteredProjects.map(p => ({
                        ...
                        '建案名稱': p.caseName,
                        '起造建商': p.builder,
                        '負責人': buildersMap[p.builder]?.representative || '--',
                        '規劃戶數': p.household,
                        '已售戶數': p.salesStats?.soldUnits || 0,
                        '銷售率(%)': p.salesStats?.salesRate || 0,
                        '平均單價(萬/坪)': p.salesStats?.avgPricePerPing || '--',
                        '申報備查日': p.declareDate,
                        '建照字號': p.permitNo,
                        '土地使用分區': p.useZoning
                    }));
                    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(rows), '建案檢索結果');
                    XLSX.writeFile(wb, '宜蘭建案檢索列表.xlsx');
                }
            } catch (e) {
