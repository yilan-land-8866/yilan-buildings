            ...
            try {
                ...
                if (currentView === 'update') {
                    ...
                    allProjects.forEach(p => {
                        ...
                        hotCandidates.push({
                            project: p,
                            recentCount: s.transactions.length,
                            soldUnits: s.soldUnits,
                            totalHouseholds: p.household || s.totalHouseholds,
                            salesRate: s.salesRate,
                            avgPrice: s.avgPricePerPing
                    ...
                    hotCandidates.sort((a, b) => b.recentCount - a.recentCount);
                    const topHotRows = hotCandidates.slice(0, 10).map((item, idx) => ({
                        ...
                        '實登已售戶數': item.soldUnits,
                        '規劃總戶數': item.totalHouseholds,
                        '銷售率(%)': item.salesRate,
                        '實登均價(萬/坪)': item.avgPrice || '--'
                    }));
                    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(topHotRows), '十大熱銷建案榜');

                    const priceCandidates = allProjects.filter(p => p.salesStats && p.salesStats.avgPricePerPing > 0);
                    priceCandidates.sort((a, b) => b.salesStats.avgPricePerPing - a.salesStats.avgPricePerPing);
                    const topPriceRows = priceCandidates.slice(0, 10).map((p, idx) => ({
                        '排名': idx + 1,
                        '鄉鎮市': p.town,
                        '建案名稱': p.caseName,
                        '起造建商': p.builder,
                        '實登平均單價(萬/坪)': p.salesStats.avgPricePerPing,
                        '最高成交單價(萬/坪)': p.salesStats.maxPricePerPing || '--',
                        '規劃總戶數': p.household,
                        '銷售率(%)': p.salesStats.salesRate
                    }));
